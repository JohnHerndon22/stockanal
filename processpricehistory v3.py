...
def main():
    ...
    # (len(dfprices.index)-180)):
        ...
        fig.add_trace(go.Candlestick(x=dfprices['Date'].iloc[start_index+1:end_index],
                        ...
                        high=dfprices['High'].iloc[start_index+1:end_index],
                        low=dfprices['Low'].iloc[start_index+1:end_index],
                        close=dfprices['Close'].iloc[start_index+1:end_index], yaxis='y1',name='Candlestick'))
        ...
        fig.add_trace(go.Scatter(x=dfprices['Date'].iloc[start_index+1:end_index], y=dfprices['sma34'].iloc[start_index+1:end_index], mode='lines', name='34SMA'))
        fig.add_trace(go.Scatter(x=dfprices['Date'].iloc[start_index+1:end_index], y=dfprices['sma5'].iloc[start_index+1:end_index], mode='lines', name='5SMA'))
        
        # hurst=dfprices['hurst'].iloc[start_index+1:end_index]    
        fig.add_trace(go.Scatter(x=dfprices['Date'].iloc[start_index+1:end_index], y=dfprices['hurst'].iloc[start_index+1:end_index], name='hurst', mode='lines', yaxis='y2'))
        ...
        fig.update_yaxes(title_text="<b>secondary</b> Hurst", range=[0, 1], secondary_y=True)
